...
use AddonPayments\Api\Entities\Enums\EncyptedMobileType;
use AddonPayments\Api\PaymentMethods\RecurringPaymentMethod;
use AddonPayments\Api\PaymentMethods\AlternativePaymentMethod;
use AddonPayments\Api\Entities\Customer;
use AddonPayments\Api\Entities\Enums\RecurringSequence;
use AddonPayments\Api\Entities\Enums\RecurringType;
use AddonPayments\Api\Entities\Enums\DccProcessor;
use AddonPayments\Api\Entities\Enums\DccRateType;
use AddonPayments\Api\Entities\DccRateData;
use AddonPayments\Api\Entities\DccResponseResult;
use AddonPayments\Api\Entities\FraudManagementResponse;
use AddonPayments\Api\Entities\AlternativePaymentResponse;
...
class RealexConnector extends XmlGateway implements IPaymentGateway, IRecurringService, ISecure3dProvider
{
    ...
    public function serializeRequest(AuthorizationBuilder $builder)
    {
        ...
        if (!empty($builder->hostedPaymentData)) {
            ...
            $this->setSerializeData('HPP_CUSTOMER_PHONENUMBER_HOME', $builder->hostedPaymentData->customerHomeNumber);
            $this->setSerializeData('HPP_CUSTOMER_PHONENUMBER_WORK', $builder->hostedPaymentData->customerWorkNumber);
            $this->setSerializeData('HPP_CARDHOLDER_ACCOUNT_AGE_DATE', $builder->hostedPaymentData->accountAgeDate);
            $this->setSerializeData('HPP_CARDHOLDER_ACCOUNT_AGE_INDICATOR', $builder->hostedPaymentData->accountAgeIndicator);
            $this->setSerializeData('HPP_CARDHOLDER_ACCOUNT_CHANGE_DATE', $builder->hostedPaymentData->accountChangeDate);
            $this->setSerializeData('HPP_CARDHOLDER_ACCOUNT_CHANGE_INDICATOR', $builder->hostedPaymentData->accountChangeIndicator);
            $this->setSerializeData('HPP_CARDHOLDER_ACCOUNT_PASSWORD_CHANGE_DATE', $builder->hostedPaymentData->accountPassChangeDate);
            $this->setSerializeData('HPP_CARDHOLDER_ACCOUNT_PASSWORD_CHANGE_INDICATOR', $builder->hostedPaymentData->accountPassChangeIndicator);
            $this->setSerializeData('HPP_CARDHOLDER_ACCOUNT_PURCHASE_COUNT', $builder->hostedPaymentData->accountPurchaseCount);
            $this->setSerializeData('HPP_TRANSACTION_TYPE', $builder->hostedPaymentData->transactionType);
            $this->setSerializeData('HPP_CARDHOLDER_ACCOUNT_IDENTIFIER', $builder->hostedPaymentData->cardholderAccountIdentifier);
            $this->setSerializeData('HPP_SUSPICIOUS_ACTIVITY', $builder->hostedPaymentData->suspiciousActivity);
            $this->setSerializeData('HPP_PROVISION_ATTEMPTS_DAY', $builder->hostedPaymentData->provisionAttemptsDay);
            $this->setSerializeData('HPP_PAYMENT_ACCOUNT_AGE', $builder->hostedPaymentData->paymentAccountAge);
            $this->setSerializeData('HPP_PAYMENT_ACCOUNT_AGE_INDICATOR', $builder->hostedPaymentData->paymentAccountAgeIndicator);
            $this->setSerializeData('HPP_DELIVERY_EMAIL', $builder->hostedPaymentData->deliveryEmail);
            $this->setSerializeData('HPP_DELIVERY_TIMEFRAME', $builder->hostedPaymentData->deliveryTimeframe);
            $this->setSerializeData('HPP_SHIP_INDICATOR', $builder->hostedPaymentData->shipIndicator);
            $this->setSerializeData('HPP_SHIPPING_ADDRESS_USAGE', $builder->hostedPaymentData->shippingAddressUsage);
            $this->setSerializeData('HPP_SHIPPING_ADDRESS_USAGE_INDICATOR', $builder->hostedPaymentData->shippingAddressUsageIndicator);
            $this->setSerializeData('HPP_SHIPPING_NAME_INDICATOR', $builder->hostedPaymentData->shippingNameIndicator);
            $this->setSerializeData('HPP_PREORDER_DATE', $builder->hostedPaymentData->preorderDate);
            $this->setSerializeData('HPP_PREORDER_PURCHASE_INDICATOR', $builder->hostedPaymentData->preorderPurchaseIndicator);
            $this->setSerializeData('HPP_REORDER_ITEM_INDICATOR', $builder->hostedPaymentData->reorderItemIndicator);
            $this->setSerializeData('HPP_TRANSACTION_ACTIVITY_DAY', $builder->hostedPaymentData->transactionActivityDay);
            $this->setSerializeData('HPP_TRANSACTION_ACTIVITY_YEAR', $builder->hostedPaymentData->transactionActivityYear);
            $this->setSerializeData('HPP_GIFT_CARD_AMOUNT', $builder->hostedPaymentData->giftCardAmount);
            $this->setSerializeData('HPP_GIFT_CARD_COUNT', $builder->hostedPaymentData->giftCardCount);
            $this->setSerializeData('HPP_GIFT_CARD_CURRENCY', $builder->hostedPaymentData->giftCardCurrency);
            $this->setSerializeData('HPP_RECURRING_MAX_INSTALLMENTS', $builder->hostedPaymentData->recurringMaxInstallments);
            $this->setSerializeData('HPP_RECURRING_EXPIRY', $builder->hostedPaymentData->recurringExpiry);
            $this->setSerializeData('HPP_RECURRING_FREQUENCY', $builder->hostedPaymentData->recurringFrequency);
            $this->setSerializeData('HPP_PRIOR_TRANSACTION_AUTHENTICATION_METHOD', $builder->hostedPaymentData->priorTransAuthMethod);
            $this->setSerializeData('HPP_PRIOR_TRANSACTION_AUTHENTICATION_IDENTIFIER', $builder->hostedPaymentData->priorTransAuthIdentifier);
            $this->setSerializeData('HPP_PRIOR_TRANSACTION_AUTHENTICATION_TIMESTAMP', $builder->hostedPaymentData->priorTransAuthTimestamp);
            $this->setSerializeData('HPP_PRIOR_TRANSACTION_AUTHENTICATION_DATA', $builder->hostedPaymentData->priorTransAuthData);
            $this->setSerializeData('HPP_CARDHOLDER_LOGIN_AUTHENTICATION_TYPE', $builder->hostedPaymentData->cardLoginAuthType);
            $this->setSerializeData('HPP_CARDHOLDER_LOGIN_AUTHENTICATION_TIMESTAMP', $builder->hostedPaymentData->cardLoginAuthTimestamp);
            $this->setSerializeData('HPP_CARDHOLDER_LOGIN_AUTHENTICATION_DATA', $builder->hostedPaymentData->cardLoginAuthData);
            $this->setSerializeData('HPP_WHITELIST_STATUS', $builder->hostedPaymentData->whiteListStatus);
